...
//=============================================
//let's create 4H candles

//there are i objects in class Candle[] with last candle[i-1]
int num4h = (i)/16;//number of 4h candles
                  //digits after dot will be removed

Records Candle4h[num4h+1];//not working having declared 20000
                     //1 additional candle is ongoing

//length of ongoing candles in terms of number of 15m candles
int rem15m = (i)- (num4h)*16;

//=============================================
//couting for test
cout << "there are 15m candles: "<< i << endl;
cout << "there are 4h candles: "<< num4h << endl;
cout << "remaining 15m candles: "<<rem15m << endl;
//=============================================

k=0;//was declared before
int k1=0;
int hight=16;//16 candles or less in case of ongoing 4H candle
//=============================================
while (k<(num4h+1)) {        //has to be while (k<(num4h+1)) 

      if (k==num4h) {  //do we have last ongoing candle?
              hight =rem15m;
      }

      Candle4h[k].Time=Candle[k1].Time;
      Candle4h[k].Open =Candle[k1].Open ;
      Candle4h[k].Close =Candle[k1+(hight-1)].Close ;
      Candle4h[k].Real =Candle[k1].Real ;//workaraund, as Real
...
cout <<Candle4h[k].Time << "   " << Candle4h[k].Open << "   " 
     <<Candle4h[k].Close << "   " << Candle4h[k].High << "   " 
     <<Candle4h[k].Low << "   " << Candle4h[k].Volume  <<"   " 
     <<Candle4h[k].Real << endl;
...
int TD[num4h+1];//indluding 1 ongoing, from 0 to num4h
...
j=0;//was declared
while (j<4) {   
     string tempdate = candledate(Candle4h[j].Time);
      outfile << j << ","<<Candle4h[j].Time << ","<< tempdate<<"," << TD[j] << endl;
++j;
}

//=============================

while (k<(num4h+1)) {           //has to be while (k<num4h) 
   //check direction (up or down)
   //=============================
   double dif = Candle4h[k].Close - Candle4h[k-4].Close; //can 
...
string tempdate = candledate(Candle4h[k].Time);
outfile << k << "," <<Candle4h[k].Time <<","<< tempdate <<","<< TD[k] << endl;
...
string GR12[num4h+1];//plus 1 ongoing
int Real[num4h+1];//plus 1 ongoing
...
//=============================================
while (k<(num4h)) {           //has to be while (k<num4h) 
   ...
   double curhigh = Candle4h[k].High;
   double curlow = Candle4h[k].Low;

   double back3close = Candle4h[k-3].Close;
   double nexthigh = Candle4h[k+1].High;
   double nextlow = Candle4h[k+1].Low;
...
if(current==1){ //don't forget that here we use ==
    ...
    if(back3close<curhigh) {
        ...
        //=============================================
        outfile << TD[k]  << "," <<Candle4h[k].Time << ","<< candledate(Candle4h[k].Time)<<","<< GR12[k]<<","<<Real[k]<<endl;
        //=============================================
...
if(next2==1){ 
   current=TD[k];
   next=TD[k+1];
   curhigh = Candle4h[k].High;
   back3close = Candle4h[k-3].Close;
   nexthigh = Candle4h[k+1].High;
...
if(next2==1){ 
   current=TD[k];
   next=TD[k+1];
   curlow = Candle4h[k].Low;
   back3close = Candle4h[k-3].Close;
   nextlow = Candle4h[k+1].Low;
...
profit[m].Body = Candle[n+15].Close-Candle[n].Open;
...
//WORKARAUND NO.1
//########################
    //if this nn gave long signal, check whether Low is not the opening of the candle[TO BE TESTED]
     if (ii==0){
         if(nn==startlong) { 
            if (Candle[nn].High>profitusd[ii]) {
...
//WORKARAUND NO.2
//########################
//if stop loss was recorded, check whether previous profit taking level was reached at the same candle, and if yes, and candle closed above current stop level, then make ++n
//something like 
      int case2=0;
      if(ii>0) {     
         if (nn==nn2[ii-1]) {//
...
//WORKARAUND NO.3
//########################
//if stop loss was recorded, check whether low/close trigers stop, and if yes, then check whether profit level was reached at high [TO BE TESTED]
//something like 
      if (case2 == 0){
          if(ii>0) {
              if (Candle[nn].High>profitusd[ii]) {
                  if (Candle[nn].Open>Candle[nn].Close) {
...
profit[m].Body = Candle[n].Open - Candle[n+15].Close;
...
//WORKARAUND NO.1
//########################
    //if this nn gave SHORT signal, check whether High is not the opening of the candle[TO BE TESTED]
     if (ii==0){
         if(nn==startshort) { 
            if (Candle[nn].Low<profitusd[ii]) {
                 cout<<"marginal case no.1.1(short): "<<nn<<endl;
                 profit[m].Profit[ii]=proflev[ii]-3-lag;
                 catch1=0;
             }
             if (Candle[nn].Close<stopusd[ii]) {
                 cout<<"marginal case no.1.2(short): "<<nn<<endl;
                 --ii;
                 ++nn; //you should put it back into nn loop
                 catch1=0;
             }
...
//WORKARAUND NO.2
//########################
//if stop loss was recorded, check whether previous profit taking level was reached at the same candle, and if yes, and candle closed BELOW current stop level, then make ++n
//something like 
      int case2=0;
      if(ii>0) {     
         if (nn==nn2[ii-1]) {//
             if (Candle[nn].Close<stopusd[ii]) {
                 cout<<"marginal case no.2(short): "<<nn<<endl;
                 --ii;
                 ++nn; //you should put it back into nn loop
                 catch1=0;
                 case2=2;
...
//WORKARAUND NO.3
//########################
//if stop loss was recorded, check whether HIGH/close trigers stop, and if yes, then check whether profit level was reached at LOW [TO BE TESTED]
//something like 
//ERROR due to duplication with no.2
      if (case2 == 0){
          if(ii>0) {
              if (Candle[nn].Low<profitusd[ii]) {
                  if (Candle[nn].Open<Candle[nn].Close) {
                     cout<<"marginal case no.3(short): "<<nn<<endl;
                     profit[m].Profit[ii]=proflev[ii]-3-lag;
                     //don't add nn, maybe next pofit can be taken 
                     //on this nn
                     catch1=0;
...
//from here you can write class profit to csv

//=============================================
//#############################################
//now you need loop to find a candle in 15min candles class
...
//identify whether take profit levels were achieved

//calculate profit/loss of each trade
